...
import KeyboardArrowDownIcon from "@mui/icons-material/KeyboardArrowDown";
import KeyboardArrowUpIcon from "@mui/icons-material/KeyboardArrowUp";
import wafrIcon from '../../assets/img/sidebar/wafr.png'
import llmIcon from '../../assets/img/sidebar/llm.png'
import {
  deleteSelectedInfraTopologyView,
  deleteActiveTab,
  deleteUrlDetailsOfPage,
          ...
          <List>
            {sideBarMenu.menu.map((item) => {
              return (
                ...
                >
                  <Link
                    to={`${APP_PREFIX_PATH}${item?.subMenu?.[0]?.link ? item?.subMenu[0]?.link : item.link}`}
                    onClick={() => {
                      deleteSelectedInfraTopologyView();
                      deleteActiveTab();
                      deleteUrlDetailsOfPage();
                  ...
                  >
                    {/* Check if it's WAFR or LLM to display the image with a tooltip */}
                    {item.name === "WAFR" ? (
                      <HtmlTooltip
                        className="sidebar-tooltip"
                        placement="right"
                        title={Parser(item.name)} // Tooltip for WAFR
                      >
                        <img src={wafrIcon} alt="WAFR Icon" className="sidebar-icon" />
                      </HtmlTooltip>
                    ) : item.name === "LLM" ? (
                      <HtmlTooltip
                        className="sidebar-tooltip"
                        placement="right"
                        title={Parser(item.name)} // Tooltip for LLM
                      >
                        <img src={llmIcon} alt="LLM Icon" className="sidebar-icon" />
                      </HtmlTooltip>
                    ) : (
                      <HtmlTooltip
                        className="sidebar-tooltip"
                  ...
                  {item.subMenu && (
                    <List
                      className={`${isActiveSubMenu ? 'submenuopen' : 'submenuclose'}`}
                    >
                      {isActive ? '' : <Box className="submenu-title">{Parser(item.name)}</Box>}
                      {item.subMenu &&
                        item.subMenu.map((subItem) => (
                          <ListItem
                            key={v4()}
                            className={currentLocation.includes(subItem.link) ? 'active' : ''}
                          >
                            <Link to={`${APP_PREFIX_PATH}${subItem.link}`}>
                              {Parser(subItem.name)}
                              <span>
